...
D_E_X.Parent = game:FindFirstChildOfClass("CoreGui")
task.spawn(function()
	local Gui = D_E_X
...
Mouse.Button1Down:connect(function()
	if CurrentWindow == "Explorer" and Settings.ClickSelect then
		...
		if target then
			bindSetSelection:Invoke({target})
		end
	end
end)
...
local Selection do
	...
	bindSetSelection = explorerPanel:FindFirstChild("TotallyNotSetSelection")
...
function rightClickMenu(sObj)
	...
	currentRightClickMenu = CreateRightClickMenu(
		{"Cut","Copy","Copy Path","Paste Into","Duplicate","Delete","Group","Ungroup","Select Children","Teleport To","Insert Part","Insert Object","View Script","Save Instance","Call Function","Call Remote"},
		"",
		false,
		function(option)
			if option == "Cut" then
			...
			elseif option == 'Copy Path' then
				local Result = ''
				for Index, Instance in next, Selection.List do
					Result = Result .. GetFullName(Instance) .. (Index ~= #Selection.List and '\n\n' or '')
				end
				setclipboard(Result)
			end
	end)
